...
export default function EmergencyDetailsScreen() {
  ...
  const loadContact = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await api.getEmergencyContact(id);
      setContact(data);
    } catch (error: any) {
  ...
  const renderContactSkeleton = () => (
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.card}>
        {[1, 2, 3].map((item) => (
          <View key={`contact-info-skeleton-${item}`} style={styles.skeletonInfoRow}>
      ...
      <View style={styles.card}>
        {[1, 2, 3, 4].map((item) => (
          <View key={`contact-access-skeleton-${item}`} style={styles.skeletonAccessRow}>
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <AccessRow label="Secure notes" allowed={contact.allowNotes} C={C} />
        </View>
...
function AccessRow({ label, allowed, C }: any) {
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: label === 'Secure notes' ? 0 : 1, borderBottomColor: C.border }}>
...
const makeStyles = (C: any) => StyleSheet.create({
  ...
  skeletonAvatar: { width: 78, height: 78, borderRadius: 24, marginBottom: 14 },
  skeletonTitle: { width: '62%', height: 26, marginBottom: 10 },
  skeletonSubtitle: { width: '74%', height: 13, marginBottom: 22 },
  skeletonInfoRow: { paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: C.border },
  skeletonInfoLabel: { width: '34%', height: 11, marginBottom: 8 },
  skeletonInfoValue: { width: '62%', height: 15 },
  skeletonSectionTitle: { width: '42%', height: 18, alignSelf: 'flex-start', marginBottom: 10 },
  skeletonAccessRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: C.border },
  skeletonAccessText: { width: '42%', height: 14 },
  skeletonAccessIcon: { width: 22, height: 22, borderRadius: 8 },
  skeletonButton: { width: '100%', height: 52, borderRadius: 999, marginTop: 2 },
  skeletonButtonDanger: { width: '100%', height: 52, borderRadius: 999, marginTop: 12 },
  ...
  avatar: { width: 78, height: 78, borderRadius: 39, backgroundColor: C.primary, alignItems: 'center', justifyContent: 'center', marginBottom: 14 },
  ...
  card: { width: '100%', backgroundColor: C.backgroundElement, borderRadius: 20, borderWidth: 1, borderColor: C.border, paddingHorizontal: 16, marginBottom: 20 },
  noteCard: { width: '100%', backgroundColor: C.backgroundElement, borderRadius: 18, borderWidth: 1, borderColor: C.border, padding: 16, marginBottom: 20 },
  noteText: { color: C.text, fontSize: 14, lineHeight: 21 },
  mainButton: { width: '100%', backgroundColor: C.backgroundbutton, borderRadius: 999, paddingVertical: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9, marginTop: 2 },
  mainButtonText: { color: '#fff', fontSize: 15, fontWeight: '900' },
  deleteButton: { width: '100%', backgroundColor: C.alertDangerBg, borderColor: C.danger, borderWidth: 1, borderRadius: 999, paddingVertical: 15, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9, marginTop: 12 },
