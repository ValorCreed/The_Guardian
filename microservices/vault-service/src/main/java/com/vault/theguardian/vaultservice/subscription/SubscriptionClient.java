package com.vault.theguardian.vaultservice.subscription;
...
public class SubscriptionClient {
    ...
    public boolean canCreateVaultItem(Long userId, long currentCount) {
        long limit = getEntitlements(userId).maxPasswords();
        return limit < 0 || currentCount < limit;
    }

    public boolean canCreateSecureNote(Long userId, long currentCount) {
        long limit = getEntitlements(userId).maxSecureNotes();
        return limit < 0 || currentCount < limit;
    }

    public boolean canUploadDocuments(Long userId) {
        return getEntitlements(userId).canUploadDocuments();
    }
